...
%% Ñ$\mclass{Lung}\;\mathtt{subClassOf}\;\mirel{hasPart}\;\mathtt{some}\;\mclass{LobeOfLung}$ì
...
Ñthe totality of processes taking place in the spatiotemporal region occupied by
the entity.ì (\cite{BFO2:ref}) This means that there is a one to one correspondence
between continuants 
%(on the SNAP side) 
and certain processes, which effectively
provides a Ñbridgeì between the 3D and the 4D perspective. No complete formal theory
of histories, which have previously been described in \cite{cornucopia}, is available as of yet.

\subsection{Conventional Modellersí Strategy for Temporalised Relations in OWL 2}
...
Ñfor all timesì interpretation in order to avoid obvious problems with the
...
%
%
If the underlying interpretation were Ñfor some timeì, transitivity of the
...
lead to additional expressivity. But it has ñ to our knowledge ñ never been made
...
still has the consequence that Ñ\emph{temporary relatedness}ì cannot be
...
select reification classes based on what seems ontologically Ñfittingì for the
...
\cite{Welty:2006}. And while they agree that the 4D  approach is Ñclearly 
not something that immediately appeals to common senseì, they also claim
that it Ñgives us another tool to use when solving a practical problem.ì To this
end, they present an ontology that models fluents, i.e. Ñrelations that hold
within certain time interval but not in others.ì This works by considering all
...
Ñ$\mathtt{inverseOf}(\mrelb{timeSliceOf})\;\mathtt{only}$ì clause to ensure that
...
contingent properties \cite{Zamborlini:Guizzardi}. For them, certain Ñmaterial
relationsì only hold by virtue of a separate truthmaker, the so called
\emph{relator}, which is formed by combining the Ñqua individualsì that
...
Both kinds of entities are examples of Ñmomentsì in their
...
Underneath the level of qua individuals (e.g. Ñ$\mclass{LeafQuaPartOfTree}$ì)
...
\subsection*{Temporalized Relations}

In which we describe Alan's approach and the BFO 2 Graz release in quite some detail and thereafter evaluate it. 

Of the three patterns \emph{temporary relatedness} (TR), \emph{permanent generic relatedness} (PGR), and \emph{permanent specific relatedness} (PSR), TR and PSR have in common that they can bea asserted between individuals: a token $a$ can be temporarily related to a token $b$, such as a specific leaf is temporarily part of a specific plant, or a specific human, e.g. Barack Obama, is temporarily located at a specific place, e.g. in his plane Air Force One. 
...
A major drawback here is the fact that PGR cannot be expressed by a relation between indivdual entities. For instance, the statement ``every cell nucleus is part of some cell at any time" does not mean that a certain cell nucleus part of a certain cell. It can becom part of a different cell if the original cell fuses with another one and becomes a different individual. It is therefore counterintuitive to assert PGR at the level of individuals. Therefore, the OBO ``standard case'' cannot be expressed via temproalized relations. 
...
The reasoning is the following: If we want to express that the classes $A$ and $B$ are related by a permanent generic relationship, such as $A$ $\mclass{obo:hasPart}$ $B$, we may resort to \emph{histories}, which are occurrents and can therefore be related by binary relations. Histories can be related by inclusion relations, such as the history of a cell is part of the history of an organism, as long as the cell came into and wnet out of being in the same organism. 

If we want to express that all instance of the class $A$ are always located in some instance of the class $B$ we can express it, in OWL in the following way:
%
\begin{equation}
\begin{split}
(\mirel{historyOf}\;\mathtt{some}\;\mclass{A}) \;\mathtt{subClassOf}\;\mirel{partOf}\;\mathtt{some}\; 
(\mirel{historyOf}\;\mathtt{some}\;\mclass{B})
\end{split}
\label{eq:historydl}
\end{equation}    
%
However, the inclusion of two histories does not allow a distinction between location and parthood \cite{Jansen&Schulz Mereology}, so that the relation between a pregnant organism and an embryo or fetus would not be distinguished between the relation between an organism and its brain.


\todo{This section should be revised by Alan}
...
\subsection*{Temporally Qualified Continuants}
