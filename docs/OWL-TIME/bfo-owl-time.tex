...
\author{Alan Ruttenberg$^{1}$,%
  ...
  Janna Hastings\correspondingauthor$^{2,3}$,%
...
This paper addresses an ontological puzzle that has arisen in the Open Biomedical Ontologies (OBO) Foundry community and in the current BFO 2 redesign project (CITE). However, these issues seem to be of general interest, as they addresse a weakness of a large number of ontologies that use the popular Semantic Web Ontology language family OWL and propose an upper-level distinction between entities that are always there in their entirerty (continuants) and those which are composed of temporal parts (occurrents). 

But not only do entities need to be differentiated based on their temporal mode of existence, temporal aspects also have to be taken into account when specifying the relationships between those entities. A particular problem arises when the topic domain in question includes not only static, but also dynamic aspects of reality. This is the case for most of the life sciences, where continuants like organisms, shapes, disorders not only persist, but also continuously change over time. The shape and the constitution of biological organisms are in continuous
flux. They continuously gain and lose parts, qualities, and dispositions. Consequently, any relational expression which makes reference to a particular continuant can have different truth values at different times and would therefore be ambiguous if time were not made explicit in the statement.


Our primary point of reference is the BFO (Basic Formal Ontology) top-level ontology \cite{BFO2:Graz}. In the current work on a new release of BFO (BFO 2), ternary (time-indexed) instance-level relations have
...
This specification 
%of BFO 2 
is contrasted with the language restrictions of OWL, as 
a formalism requested by most users of (computable) ontologies, which 
%. However, OWL 
conveys considerable limitations compared to FOL, however justified by the computational 
...
%for working around 
this limitation. It is organised as follows:\todo{Revise} 
...
BFO offers a small set of foundational classes, together with descriptive axioms, most of which make use of a set of foundational relationships intended to be used in multiple ontologies. Both classes and relationships are domain-independent, albeit it is clearly stated that BFO's focus is on the representation of natural and applied sciences, with biology and medicine as their most important representatives.  
Thus, domain-specific relationships such as $\mirel{isTautomerOf}$ (a chemical relationship used in ChEBI) or $\mirel{isAbout}$ (a relation in the information artifact ontology) are out of scope for BFO itself, but they should be covered by domain ontologies below BFO, according to the OBO Foundry criteria (CITE).  

BFO subscribes to three-dimensionalism. As a consequence, the uppermost partition in its class hierarchy reflects temporal mode of existence: \textit{continuants} are entities that (1) exist in full at any time that they exists at all, and (2) continue to exist self-identically for as long as they exist; \textit{occurrents} are entities that unfold over a period of time and thus have temporal parts. For example, you are a continuant, while your life is an occurrent.  A cell is a continuant, while the process of cell division is an occurrent.  
...
The Web Ontology Language (OWL), currently available as versions 1 and 2 \cite{grau2008} is an ontology development language which is supported by a wide range of tools such as editors and reasoners. Based on Description Logics (DL), a family of 
%knowledge 
representation formalisms designed to build
% large-scale
%terminological knowledge 
large scale ontologies
\cite{baader2007dlhandbook}, OWL has been standardized and popularized by the Semantic Web Community. OWL arranges classes and relations in single or multiples hierarchies, defines constraining axioms on classes (TBox) and, optionally, assertions about 
individuals (ABox). There is a broad range of DL dialects all of which are 
decidable subsets of first-order logics (FOL). 
Among the several dialects of OWL, its description logics profile 
%OWL, and particularly its language profile 
OWL-DL is increasingly being adopted by large scale biomedical
ontologies such as the Gene Ontology \cite{go2000} and ChEBI \cite{chebinar2013}. Although OWL DL had been introduced as a member of the OWL 1 family, its syntax and semantics is covered by the specification of OWL 2, as every OWL DL ontology is also an OWL 2 ontology \cite{OWL2:direct}. For the remainder of this paper we will refer to OWL DL and the OWL 2 Direct Semantics, specified by the W3C \cite{OWL2:direct}.  

\todo{NEEDS IMPROVEMENT}

A limitation of most DL-based languages, including all OWL profiles, is the restriction of relational expressions to binary relationships (object properties). However, in nearly all scientific domains
% the biomedical domain,
%much of the knowledge that is 
numerous statements that are supposed to be expressed in ontologies are linked to time.
% in some fashion. 
For example, different developmental stages of an organism have different anatomical properties. 
...
But while there has been work on expressive description logics that try to transcend this limitation \cite{Calvanese:1997} and also on description logics
that explicitly account for temporality (e.g. \cite{Wolter:2001}), there is no strong push towards standardisation of those formalisms, and tools suitable for end users are not readily available. 
%It has thus been acknowledged that there is a need for solutions 
%that work within the confines of present technologies
%by \cite{Welty:2006}, who juxtaposes a reification approach, 
%in which ternary relations are expressed as OWL classes to a four-dimensionalist approach, which allows continuants to have temporal parts.
...
\begin{itemize} 
\item UC1: Every butterfly was a caterpillar at some time (TBox, phased sortal)
\item UC2: Every electronic health record is on some computer storage at some time (TBox, permanent generic, dependent continuant)
\item UC3: Every red blood cell has some hemoglobin molecule; every hemoglobin molecule has some Fe++ ion (TBox, collectives, transitivity)
\item UC4: Every tooth is part of an organism at some time. Every portion of enamel is part of some tooth of some time. (TBox, temporary part)
\item UC5: Every human tooth is located in some human organism at some time. Every human organism is located in some uterus at some time (TBox, temporary location)
\item UC6: The blood specimen #bs4309123 was part of the blood of the patient #pat449154 (individual parthood, phased sortal (specimen))
\item UC7: On Oct 10, 2013 a lab sample was at lab 1 between 6:01 and 7:37 UTC, then it was sent to lab 2 where it remained from 9:41 and 12:15 UTC (ABox individual location)
\item UC8: Tibbles has a tail all time he exists and it is always the same tail.
\item UC9: Chris' disjointness of parts 
\end{itemize}

The following competency questions are using the use cases above. They are formulated in a way that the should provoke contradictions in the ontology. 

\begin{itemize} 
\item CQ1: (using UC1): A butterfly that is a caterpillar  
\item CQ2: (using UC2): An electronic health record that is not in any storage   
\item CQ3: (using UC3): An red blood cell without Fe++;  
\item CQ4: (using UC4): A portion of enamel that have never been part of any organism 
\item CQ5: (using UC5): Every human tooth is located in some uterus at some time
\item CQ6: (using UC6): A blood specimen that was never part of any organism
\item CQ7: (using UC6): The lab sample was at lab 1 and at lab 2. The labs do not share any parts or spaces.
\item CQ8: (using UC6): Tibbles' original tail was replaced by another one.
...
\section*{Methods}
\todo[inline, size=\small]{If we want to have a Methods section it should be here, not at the end.}
