...
preview release available at \todo{URL}.  Nevertheless
...
solutions to the problem.\todo[inline]{Removed the following text, which was contradictory and
dead wrong (since the instance level assertion \emph{is} the property assertion): \begin{quote}Furthermore, an OWL instance level assertion
using \mrelb{rel} would be problematic, as the OWL expression
$$a\;\mrelb{rel}\;b$$
would also have to be interpreted in FOL terms as 
$$\exists a, b, t:\; \mrelt{rel}(a, b, t)$$
which is the weakest possible statement on two individuals $a$ and $b$ with regard
to the relation rel and time. This contrasts with the above formalized object
property assertion.\end{quote}}
...
time and can thus not represented in OWL 2.\todo{I got a counter model for the
OWL 2 formulation if anyone's interested.}
\section{Conclusion}
