...
TQCs:
\begin{itemize}
    \item \mirel{has\_max} (inverse \mirel{max\_of}) relates a random TQC instance to its related instance 
    with the maximal temporal extension (which may be itself)
    \item \mirel{at\_some\_time} relates a TQC with each other TQC that represents the same continuant. We have
    chosen this name for a more intuitive understanding of OWL axioms where it is employed.
    \item The relation \mirel{at\_some\_time} is equivalent to the concatenation \mirel{has\_max} $\circ$ \mirel{max\_of}
    \item \mirel{max\_of} is a subrelation of \mirel{at\_some\_time}, which is not necessary if \mirel{has\_max} is represented as a 
    reflexive relation)
    \item \mirel{has\_time} relates a TQC with its defining temporal region.
\end{itemize}
